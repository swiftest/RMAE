...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout=0.1):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([MAA(dim, heads, dim_head, dropout), FeedForward(dim, mlp_dim, dropout)]))
...
class Massformer(nn.Module):
    def __init__(self, *, num_classes=0, num_tokens=121, dim=64, depth=2, heads=4, dim_head=16, mlp_dim=256, dropout=0.2, emb_dropout=0.1):
        super(Massformer, self).__init__()
        self.conv3d = nn.Sequential(nn.Conv3d(in_channels=1, out_channels=8, kernel_size=(3, 3, 3)), nn.ReLU())
        self.conv2d = nn.Sequential(nn.Conv2d(in_channels=8*28, out_channels=64, kernel_size=(3, 3)), nn.ReLU())
        ...
        nn.init.normal_(self.pos_embedding, std=.001)
...
if __name__ == '__main__':
    inputs = torch.randn(8, 1, 30, 15, 15)
    logits = Massformer(num_classes=9, num_tokens=121)(inputs)
    print(logits.size())
