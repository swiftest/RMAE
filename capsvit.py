...
def conv3x3_bn_relu(band, out_channel=96):
    return nn.Sequential(nn.Conv2d(band, 128, 5, 1), nn.BatchNorm2d(128), nn.ReLU(inplace=True), 
                         nn.Conv2d(128, out_channel, 3, 1), nn.BatchNorm2d(out_channel), nn.ReLU(inplace=True))
...
class PrimaryCapsule(nn.Module):
    def __init__(self, in_channels, caps1_num, caps1_dim, kernel_size, stride=1):
...
class Transformer(nn.Module):
    ...
    """
    def __init__(self, caps_dim, depth, heads, dim_head, mlp_dim, dropout, caps_num):
        super().__init__()
...
class ViT(nn.Module):
    ...
    def __init__(self, caps_num, caps_dim, depth, heads, mlp_dim, dim_head=16, dropout=0.1, emb_dropout=0.1):
        ...
        self.transformer = Transformer(caps_dim, depth, heads, dim_head, mlp_dim, dropout, caps_num)
...
class Decoder(nn.Module):
    def __init__(self, caps2_dim, band, patch_size=9):
        ...
        self.fc = nn.Sequential(nn.Linear(self.in_channels, 32*3*3), nn.BatchNorm1d(32*3*3), nn.ReLU())
        self.trans_conv = nn.Sequential(nn.ConvTranspose2d(32, 128, 3, stride=1), nn.BatchNorm2d(128), nn.ReLU(), # (..., 5, 5)
                                        ...
                                        nn.ConvTranspose2d(64, band, 3, stride=2))  # (batch_size, 15, 15, 15)  
...
class CapsViT(nn.Module):
    def __init__(self, band, preliminary_layer_channels=96, num_group_norm=8, steps=8, caps1_num=16, caps1_dim=8, kernel_size=3, 
                 caps2_caps=13, caps2_dim=16):
        ...
        self.primary_caps = PrimaryCapsule(output_channel, caps1_num=caps1_num, caps1_dim=caps1_dim, kernel_size=3)
        
        caps1_caps = caps1_num * 7 ** 2  # 784
        self.vit = ViT(caps_num=caps1_caps, caps_dim=caps1_dim, depth=2, heads=4, mlp_dim=8, dropout=0.1, emb_dropout = 0.1)
        
        self.digital_cap = DigitalCapsule(caps1_caps, caps1_dim, caps2_caps, caps2_dim)
        self.decoder = Decoder(caps2_dim, band)
